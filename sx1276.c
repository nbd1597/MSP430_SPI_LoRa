...
#define RadioEvents radio_events
    ...
    switch( sx1276.Settings.State )
    {
        case RF_RX_RUNNING:
            ...
            case MODEM_FSK:
                ...
                {
                    sx1276_read_fifo( ( RxTxBuffer + sx1276.Settings.FskPacketHandler.NbBytes ), sx1276.Settings.FskPacketHandler.FifoThresh );
            ...
            case MODEM_LORA:
                ...
                //TimerStop( &RxTimeoutTimer );
                // Clear Irq
                sx1276_write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_RXTIMEOUT );
        ...
        case RF_TX_RUNNING:
            ...
            case MODEM_FSK:
                ...
                {
                    sx1276_write_fifo( ( RxTxBuffer + sx1276.Settings.FskPacketHandler.NbBytes ), sx1276.Settings.FskPacketHandler.ChunkSize );
                    sx1276.Settings.FskPacketHandler.NbBytes += sx1276.Settings.FskPacketHandler.ChunkSize;
                }
                else
                {
                    // Write the last chunk of data
                    sx1276_write_fifo( RxTxBuffer + sx1276.Settings.FskPacketHandler.NbBytes, sx1276.Settings.FskPacketHandler.Size - sx1276.Settings.FskPacketHandler.NbBytes );
                    sx1276.Settings.FskPacketHandler.NbBytes += sx1276.Settings.FskPacketHandler.Size - sx1276.Settings.FskPacketHandler.NbBytes;
                }
                break;
            case MODEM_LORA:
...
//                    TimerStop( &RxTimeoutSyncWord );
                    ...
                    sx1276.Settings.FskPacketHandler.RssiValue = -( sx1276_read( REG_RSSIVALUE ) >> 1 );

                    sx1276.Settings.FskPacketHandler.AfcValue = ( int32_t )( double )( ( ( uint16_t )sx1276_read( REG_AFCMSB ) << 8 ) |
                                                                           ( uint16_t )sx1276_read( REG_AFCLSB ) ) *
                                                                           ( double )FREQ_STEP;
                    sx1276.Settings.FskPacketHandler.RxGain = ( sx1276_read( REG_LNA ) >> 5 ) & 0x07;
                }
                break;
            case MODEM_LORA:
                if( sx1276.Settings.LoRa.FreqHopOn == true )
                {
                    ...
                    {
                        RadioEvents->FhssChangeChannel( ( sx1276_read( REG_LR_HOPCHANNEL ) & RFLR_HOPCHANNEL_CHANNEL_MASK ) );
                    }
                }
                break;
            default:
